#pragma once
#include <cstdint>
#include <cstring>

// NO namespace wrapper — this file is included inside namespace pvac already

static const uint8_t AES_SBOX[256] = {
    
};

static inline uint32_t aes_subword(uint32_t w){
    return ((uint32_t)AES_SBOX[w&0xFF])|
           ((uint32_t)AES_SBOX[(w>>8)&0xFF]<<8)|
           ((uint32_t)AES_SBOX[(w>>16)&0xFF]<<16)|
           ((uint32_t)AES_SBOX[(w>>24)&0xFF]<<24);
}
static inline uint32_t aes_rotword(uint32_t w){ return (w>>8)|(w<<24); }
static inline uint32_t gmul8(uint32_t a,uint32_t b){
    uint32_t p=0;
    for(int i=0;i<8;i++){
        if(b&1)p^=a;
        uint32_t hi=a>>7;a=(a<<1)&0xff;if(hi)a^=0x1b;b>>=1;
    }
    return p;
}

struct AesCtr256 {
    uint32_t rk[60];
    uint64_t ctr_val;
    uint64_t buf[2];
    bool has_buf;

    void init(const uint8_t key[32], uint64_t nonce){
        static const uint32_t RC[]={0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};
        for(int i=0;i<8;i++)
            rk[i]=((uint32_t)key[4*i])|((uint32_t)key[4*i+1]<<8)|
                  ((uint32_t)key[4*i+2]<<16)|((uint32_t)key[4*i+3]<<24);
        for(int i=8;i<60;i++){
            uint32_t t=rk[i-1];
            if(i%8==0) t=aes_subword(aes_rotword(t))^RC[i/8-1];
            else if(i%8==4) t=aes_subword(t);
            rk[i]=rk[i-8]^t;
        }
        ctr_val=nonce; has_buf=false;
    }

    void encrypt_block(const uint8_t in[16], uint8_t out[16]) const {
        uint8_t s[16]; std::memcpy(s,in,16);
        for(int i=0;i<4;i++){
            uint32_t w=rk[i];
            s[4*i]^=w&0xff; s[4*i+1]^=(w>>8)&0xff;
            s[4*i+2]^=(w>>16)&0xff; s[4*i+3]^=(w>>24)&0xff;
        }
        for(int r=1;r<=14;r++){
            for(int i=0;i<16;i++) s[i]=AES_SBOX[s[i]];
            uint8_t t;
            t=s[1];s[1]=s[5];s[5]=s[9];s[9]=s[13];s[13]=t;
            t=s[2];s[2]=s[10];s[10]=t;
            t=s[6];s[6]=s[14];s[14]=t;
            t=s[3];s[3]=s[15];s[15]=s[11];s[11]=s[7];s[7]=t;
            if(r<14){
                for(int c=0;c<4;c++){
                    uint8_t a0=s[c],a1=s[c+4],a2=s[c+8],a3=s[c+12];
                    s[c]  =gmul8(2,a0)^gmul8(3,a1)^a2    ^a3;
                    s[c+4]=      a0   ^gmul8(2,a1)^gmul8(3,a2)^a3;
                    s[c+8]=      a0   ^      a1   ^gmul8(2,a2)^gmul8(3,a3);
                    s[c+12]=gmul8(3,a0)^     a1   ^      a2   ^gmul8(2,a3);
                }
            }
            for(int i=0;i<4;i++){
                uint32_t w=rk[r*4+i];
                s[4*i]^=w&0xff; s[4*i+1]^=(w>>8)&0xff;
                s[4*i+2]^=(w>>16)&0xff; s[4*i+3]^=(w>>24)&0xff;
            }
        }
        std::memcpy(out,s,16);
    }

    uint64_t next_u64(){
        if(has_buf){has_buf=false;return buf[1];}
        uint8_t cb[16]={};
        std::memcpy(cb,&ctr_val,8);
        uint8_t ob[16];
        encrypt_block(cb,ob);
        std::memcpy(buf,ob,16);
        ctr_val++;has_buf=true;
        return buf[0];
    }

    void fill_u64(uint64_t* out,size_t n){
        for(size_t i=0;i<n;i++) out[i]=next_u64();
    }

    uint64_t bounded(uint64_t M){
        if(M<=1)return 0;
        uint64_t lim=UINT64_MAX-(UINT64_MAX%M);
        for(;;){uint64_t x=next_u64();if(x<lim)return x%M;}
    }
};
